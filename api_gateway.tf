...
resource "aws_api_gateway_integration" "https_to_lambda" {
  ...
  uri                  = aws_lambda_function.sentry_integration_lambda.invoke_arn
  cache_key_parameters = null
  request_parameters   = {}
  request_templates    = {}
}

resource "aws_api_gateway_deployment" "sentry_integration_api_gw_deployment" {
  rest_api_id = aws_api_gateway_rest_api.ef_to_lambda.id
...
resource "aws_api_gateway_stage" "sentry_integration_api_gw_stage" {
  deployment_id = aws_api_gateway_deployment.sentry_integration_api_gw_deployment.id
  rest_api_id   = aws_api_gateway_rest_api.ef_to_lambda.id
  stage_name    = var.env

  depends_on = [aws_cloudwatch_log_group.sentry_integration_api_gateway_log_group]
}

resource "aws_api_gateway_method_settings" "enable_logging" {
  rest_api_id = aws_api_gateway_rest_api.ef_to_lambda.id
  stage_name  = aws_api_gateway_stage.sentry_integration_api_gw_stage.stage_name
  method_path = "*/*"
